...
## 18. Generalization Test: 30-Day Mortality, and a Newton Reweighting Failure Mode

Every result in §1-§16 concerns `postop_icu` (~11.4% positive rate). To test whether the accumulated fixes generalize to a different, much rarer clinical question — not just a different sample of the same task — the same three-scale pipeline (GA2M + Newton + Armijo vs. XGBoost vs. HistGradientBoosting) was re-run against a second target: **`mortality_30d`** (all-cause death within 30 days of surgery, derived from `allcause_death_time`, ~0.35% positive rate — roughly 30x rarer than `postop_icu`).

**A data-handling pitfall caught before it produced wrong numbers**: INSPIRE's time columns (`opend_time`, `icuin_time`, `allcause_death_time`, ...) are numeric **minutes from a per-case reference epoch**, not calendar timestamps. Applying `pd.to_datetime()` to them directly — the natural first instinct — silently produces nonsense: an implausible ~15% "30-day mortality rate" for a general surgical population. Treating the difference as minutes (÷1440 for days) gives the correct, clinically plausible rate of ~0.35%. This is recorded here as a concrete warning for anyone extending this pipeline to other INSPIRE time-derived labels.

**Result**: `mortality_30d`'s AUPRC gap versus trees is far larger, and widens with scale, unlike `postop_icu`'s roughly constant small gap:

| Scale | KANBoost (GA2M+Newton+Armijo) | XGBoost | HistGradientBoosting | Relative gap |
|---|---|---|---|---:|
| Small | AUROC 0.804, AUPRC 0.0168 | AUROC 0.897, AUPRC 0.0419 | AUROC 0.871, AUPRC 0.0281 | ~2.5x worse |
| Medium | AUROC 0.905, AUPRC 0.0434 | AUROC 0.911, AUPRC 0.1835 | AUROC 0.912, AUPRC 0.1908 | ~4.4x worse |
| Large | AUROC 0.919, AUPRC 0.0462 | AUROC 0.918, AUPRC 0.2061 | AUROC 0.908, AUPRC 0.1892 | ~4.5x worse |

Two candidate explanations were tested and ruled out or confirmed by direct experiment rather than assumed:

**Capacity was tested first and ruled out.** Raising `n_hidden` uniformly to 36 (well above the `16/20/24` used elsewhere in this evaluation) made results *worse* at every scale for both targets, not better (`postop_icu` Small: AUROC 0.9244→0.9210, AUPRC 0.6804→0.6645; `mortality_30d` Small: AUROC 0.804→0.699, AUPRC 0.0168→0.0091) — with 30-51% higher fit time to boot. A capacity-per-scale reduction (`n_hidden=8/16/24` with more rounds, `40/64/81`) helped `postop_icu` and `mortality_30d` specifically at Small scale (best result found for both targets at that scale) but didn't clearly generalize to Medium/Large. Capacity tuning alone does not close the gap.

**Newton reweighting was tested second and confirmed as the dominant cause.** A clean 2x2 experiment (Newton on/off x class-balancing on/off, capacity held fixed at the original `16/20/24` config) on `mortality_30d`, all three scales:

| Scale | Baseline (Newton, no balance) | **No Newton** | Newton + balanced | No Newton + balanced |
|---|---|---|---|---|
| Small | AUROC 0.804, AUPRC 0.0168 | **AUROC 0.887, AUPRC 0.0433** | AUROC 0.737, AUPRC 0.0150 | AUROC 0.854, AUPRC 0.0333 |
| Medium | AUROC 0.905, AUPRC 0.0434 | **AUROC 0.910, AUPRC 0.0518** | AUROC 0.869, AUPRC 0.0339 | AUROC 0.920, AUPRC 0.0437 |
| Large | AUROC 0.919, AUPRC 0.0462 | **AUROC 0.914, AUPRC 0.0535** | AUROC 0.897, AUPRC 0.0368 | AUROC 0.920, AUPRC 0.0492 |

Disabling Newton reweighting alone gives the best AUPRC at every single scale (+158% at Small, +19% at Medium, +16% at Large over the baseline), while adding class balancing *on top of* Newton makes results worse at every scale. Root cause: at this extreme a base rate (~0.35%), `hess = p(1-p)` collapses to near-zero for the overwhelming majority of confidently-negative samples, so the `min_hessian` floor dominates the reweighted target for nearly all training data — Newton reweighting, which was neutral-to-beneficial for `postop_icu`'s more moderate ~11% base rate (§9, §13), becomes actively counterproductive at this level of rarity.

**Practical guidance established by this test**: `use_newton=True` (the default recommendation from §13's Variant A) should **not** be applied unconditionally to targets with extreme class imbalance (roughly <1% positive rate, as a rule of thumb pending further testing) — disable it for such targets specifically. This is a genuine, previously-undocumented boundary condition on this evaluation's own recommendations, found only because the pipeline was deliberately tested against a target it was not tuned on.

**Interpretability held up across both targets**: `main_effect_contributions()`/`pairwise_interaction_contributions()` remained sensible for `mortality_30d` too (same dominant features as `postop_icu`: `icd10_pcs`, `department`, `age`), and the full walkthrough — including per-patient local explanation via additive decomposition of a single prediction into its main-effect and pairwise-interaction terms — is available as a runnable Colab notebook, `examples/kanboost_inspire_colab_walkthrough.ipynb`.
...
## 20. Closing Part of the Accuracy Gap: Gain-Ranked Interaction Selection (and Five Rejected Attempts First)

Following the `mortality_30d` generalization test (§18), a literature review targeted the remaining `postop_icu` accuracy gap vs. trees directly, informed by three sources: Grinsztajn et al. (NeurIPS 2022, why trees beat smooth/neural models on tabular data — irregular target functions, uninformative-feature robustness, rotation invariance), CatBoost's ordered boosting (Prokhorenkova et al., NeurIPS 2018, prediction shift), and GB-KAN (Mohr & Fröchte, ICAART 2026, the directly comparable KAN-as-weak-learner boosting paper). Five hypotheses were tested with real evidence on INSPIRE and **rejected or left inconclusive**, each isolating a different candidate mechanism (full detail, tables, and exact numbers in `AI_REVIEW_LOOP.md`, CC-24 through CC-29):

- **Row subsampling** (stochastic gradient boosting, Friedman 2002 — kanboost has none, unlike every mature boosting library): no measurable effect (Δ AUPRC ≈ 0.0000).
- **Reduced B-spline smoothness** (`kan_k=1` vs. default 3, testing Grinsztajn's smooth-bias finding directly): made accuracy *worse* (Δ AUPRC −0.0054), the opposite of the hypothesis.
- **Chebyshev polynomial basis instead of B-spline** (TabKAN's approach), tested on synthetic data with a deliberate hard discontinuity, confirmed independently on Kaggle: B-spline's local support beat Chebyshev's global support even on data engineered to favor the swap (Δ AUPRC −0.0090).
- **Approximate ordered/cross-fitted boosting** (CatBoost-style prediction-shift mitigation), two attempts including a diagnosed-and-fixed implementation bug: both negative (Δ AUPRC ≈ −0.02), the fix not changing the outcome.
- **A hybrid node-decision/edge-learning gate** at GA2M's hidden units (user-originated idea: trees decide at nodes, KAN learns on edges) — a fixed-parameter version was catastrophically negative (Δ AUPRC as low as −0.14); the fully learnable version (per-node learned blend/sharpness/threshold, Adam-optimized) gave the session's first non-negative result (Δ AUPRC +0.0036) but only ~1x the CV noise level and ~10x slower to fit — inconclusive, not pursued further.

**What worked**: an independent research pass (a second model instance, prompted with this session's full experimental history and asked for its own best-reasoned next idea) identified that kanboost implements GA2M's *model class* (Lou et al., KDD 2013) without GA2M's own *term-selection mechanism* — `fit_with_ga2m`'s pairwise-interaction hidden units are chosen uniformly at random each round, rather than by a gain/ranking criterion, the one Grinsztajn mechanism (uninformative-feature robustness) the five rejected attempts above never actually tested.

**Design**: each round, score a candidate pool of pairs by `gain = b'(M⁻¹)b` — the penalized-projection squared-error reduction achievable by that pair's own B-spline basis block alone against the current residual (a per-block version of the same closed-form quantity the existing joint solve already computes) — and keep only the top `n_pairs_per_round` by gain. Main effects unchanged (always included). Same final model capacity as the random-selection baseline (a pure selection swap, not a capacity increase); stays entirely within the existing closed-form solve (no gradient optimizer).

**Result** (Medium scale, `postop_icu`, 3 seeds × 3-fold `StratifiedGroupKFold` by `subject_id`, after a seed-fairness fix and a row-subsampled scoring optimization identified by an independent code review of the first version):

| | AUROC | AUPRC | fit time/seed |
|---|---|---|---|
| Random selection (shipped `fit_with_ga2m`) | 0.9419 ± 0.0002 | 0.7548 ± 0.0007 | 42.7s |
| Gain-ranked selection | 0.9430 ± 0.0002 | 0.7617 ± 0.0006 | 63.8s |

Δ AUPRC +0.0069, **7.53× the combined CV standard deviation** — the first result in this entire line of investigation (six hypotheses tested) to clear a pre-registered significance bar with a comfortable margin. Selected pairs were highly consistent across every round/seed/fold (the top pair was chosen in over a third of all possible round-slots), evidence of genuine, stable interaction discovery rather than selection noise. Fit-time cost is a modest 1.45× the baseline. Three further refinements proposed by a second independent review round (exhaustive candidate pool, FAST-correct conditional gain, anti-collapse stochastic selection) were tested in ablation and did **not** improve on this result on real INSPIRE data — a theory/experiment mismatch recorded honestly rather than adopted on reasoning alone.

**Status**: evidence-stage only, not yet implemented in `kanboost/train/ga2m.py`. Per this project's standing pipeline, this requires independent code/math review and scientific judgment before any merge, and explicit user approval regardless of outcome — see `AI_REVIEW_LOOP.md` (CC-30) for the full record.
