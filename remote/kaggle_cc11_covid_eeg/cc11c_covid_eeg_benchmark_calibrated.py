"""
CC-11 second correction: the fixed comparison (cc11b_...) still had an
uncontrolled capacity/budget mismatch -- HistGBDT's max_iter=300 was
actually fully used (no early stopping triggers at n=173), while KANBoost
kept CX-18's ds004504-tuned n_estimators=80 unchanged, never recalibrated
for this different dataset. Same class of error as the feature-selection
bug: reusing a borrowed config instead of calibrating fresh (see
AI_REVIEW_LOOP.md CC-11 for both).

Fix: a short calibration pass (not exhaustive grid, per the burden-of-
proof rule) on a single held-out split picked kan_hidden=4,
n_estimators=150, kan_steps=8 over 4 alternatives. Reruns the full
StratifiedKFold(5) x 5-seed comparison with this config, both models
still on identical SelectKBest(80) features.
"""
...
def main():
    ...
    rows.extend(eval_model("kanboost_e150_h4_s8_select80", X, y,
                            lambda seed: KANBoostClassifier(n_estimators=150, kan_hidden=4, kan_steps=8,
    ...
    prefix = f"cc11c-covid-eeg-calibrated_{STAMP}"
